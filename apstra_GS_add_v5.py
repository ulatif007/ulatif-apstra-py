...
url = 'https://' + aos_server + '/api/design/logical-devices'
headers = { 'AuthToken':auth_token, 'Content-Type':"application/json", 'Cache-Control':"no-cache" }
response = requests.request('GET', url, data=data, headers=headers, verify=False)
ld_dict = response.json()
response.close()

for ld_data in ld_dict.get('items'):
   if ld_data.get('display_name')=='AOS_10x1G_10x10G_10x25G':
     ld_id = ld_data.get('id')
...
with open(CSV_Filename, 'r') as read_obj:
   ...
   for row in csv_reader:
     ...
     logical_device = {"panels": [{"panel_layout": {"row_count": 2, "column_count": 15 }, "port_indexing": { "order": "T-B, L-R", "start_index": 1,"schema": "absolute" }, "port_groups": [{ "count": 10, "speed": { "unit": "G", "value": 1 }, "roles": [ "leaf", "access" ]}, { "count": 10, "speed": { "unit": "G", "value": 10 }, "roles": [ "leaf", "access" ]}, { "count": 10, "speed": { "unit": "G", "value": 25 }, "roles": [ "leaf", "access"]}]}], "display_name": "AOS_10x1G_10x10G_10x25G","id": ld_id}
     ...
     print('Creation of GS messages returned from Apstra as follows:')
...
# Form LAG interfaces for interfaces with LACP
     ...
     if lag_mode!='none': 
       ...
       print('Creation of LAG messages returned from Apstra as follows:')
       print(response.status_code)
       print(response.text)
       print('\n')
       response.close()
     ...
     task_check -= 1
     if task_check==0:
       input('\nCheck Task List in Apstra and press ENTER if pending tasks are zero')
       task_check=20

print('\nGS definition and Leaf links have been staged\n')
print('\nStandby 120 seconds before starting CT assignment ...\n')
         ...
         for listdict in cm_dict.get('links'):
            if listdict.get('role')=='to_generic':
              if listdict.get('endpoints')[0]['system']['label']==leaf1_hname and listdict.get('endpoints')[0]['interface']['if_name']==leaf1_port:
                ...
                for lstdict in cm_dict.get('links'):
     ...
     for ct_name in ct_list:
        for listdict in ctpolicies.get('endpoint_policies'):
           if listdict.get('label')==ct_name and listdict.get('policy_type_name')=='batch':
             ...
             policy_list.append({"policy": ct_id, "used": True})

 
     for ct_intf in intfdata:
        ct_details = {"application_points": [{"id": ct_intf, "policies": policy_list}]}
        ...
        print('CT assignment messages returned from Apstra as follows:')
        print(response.status_code)
        print(response.text)
        response.close()
